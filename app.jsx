...
function checkAchievements(hero, completedQuestIds, history, currentAchievements) {
  ...
  if (!updated.a1 && (hero.totalCompleted || 0) >= 1) updated.a1 = true;
  if (!updated.a2 && (hero.streak || 0) >= 7)         updated.a2 = true;
  if (!updated.a5 && (hero.streak || 0) >= 30)        updated.a5 = true;
  if (!updated.a6 && hero.level >= 20)                updated.a6 = true;
  if (!updated.a7 && hero.gold >= 5000)               updated.a7 = true;
  const guildCount = history.filter(h => h.stat === 'carisma').length;
  if (!updated.a8 && guildCount >= 10)                updated.a8 = true;
  return updated;
}
...
function App() {
  ...
  const toggleMode = () => setTweak('mode', tweaks.mode === 'night' ? 'day' : 'night');

  // ModeContext value — memo para evitar re-renders innecesarios
  const modeCtx = useMemo(() => ({ mode: tweaks.mode, toggleMode }), [tweaks.mode]);
    ...
    const unsubscribe = firebaseAuth.onAuthStateChanged(async (fbUser) => {
      if (fbUser) {
        try {
          ...
          // Actualizar foto de Google si cambió
          if (fbUser.photoURL && userData.hero.photoURL !== fbUser.photoURL) {
            userData.hero = { ...userData.hero, photoURL: fbUser.photoURL };
          }
          ...
          // Cargar gremio si existe
          let guildData = null;
          if (userData.guildId) {
            try { guildData = await fbLoadGuild(userData.guildId); } catch {}
          }
          ...
          setGuild(guildData);
          setGuildId(userData.guildId || null);
          setScreen('main');
        } catch (err) {
        ...
        setUser(null); setHero(null);
        setCompletedIds([]); setHistory([]);
        setGuild(null); setGuildId(null);
        setScreen('login');
      }
    });
    return () => unsubscribe();
  ...
  // ── Navegación ─────────────────────────────────────────────
  const onOpenQuest  = (q) => { setOpenQuest(q); setScreen('detail'); };
  ...
  const onCompleteQuest = async (q) => {
    ...
    let leveledUp = false;
    let updatedHero = { ...hero, gold: hero.gold + q.gold, totalCompleted: (hero.totalCompleted || 0) + 1 };

    // Subir stat relacionada
    if (updatedHero.stats?.[q.stat]) {
      updatedHero.stats = {
        ...updatedHero.stats,
        [q.stat]: { ...updatedHero.stats[q.stat], value: updatedHero.stats[q.stat].value + 1 },
      };
    }

    if (newXp >= hero.xpNext) {
      leveledUp = true;
      updatedHero = { ...updatedHero, level: hero.level + 1, xp: newXp - hero.xpNext, xpNext: Math.round(hero.xpNext * 1.5) };
    } else {
      updatedHero.xp = newXp;
    }

    // Racha: primera misión del día
    if (completedQuestIds.length === 0) {
      updatedHero.streak = (hero.streak || 0) + 1;
      if (updatedHero.streak > (hero.bestStreak || 0)) updatedHero.bestStreak = updatedHero.streak;
    }
    ...
    const newHistory = [
      { date: new Date().toLocaleDateString('es', { weekday:'long', day:'numeric', month:'short' }),
        ...
        epic: q.type === 'epica', completedAt: new Date().toISOString() },
      ...history,
    ].slice(0, 50);

    const updatedAch = checkAchievements(updatedHero, newCompletedIds, newHistory, achievements);
    const achChanged = JSON.stringify(updatedAch) !== JSON.stringify(achievements);

    setHero(updatedHero);
    setCompletedIds(newCompletedIds);
    setHistory(newHistory);
    if (achChanged) setAchievements(updatedAch);
    setReward({ quest: q, leveledUp });
    setScreen('main');

    if (user) {
      try {
        await fbCompleteQuest(user.uid, q, updatedHero, newCompletedIds, achChanged ? updatedAch : null);
        // Actualizar resumen en gremio
        if (guildId) {
          try { await fbUpdateGuildMember(guildId, user.uid, updatedHero); } catch {}
        }
      } catch (err) { console.error('Error guardando misión:', err); }
    }
  ...
  // ── Guild callbacks ─────────────────────────────────────────
  const onGuildUpdate = (newGuild, newGuildId) => {
    setGuild(newGuild);
  ...
  if (screen === 'loading') {
    return (
      <div className="wood grain" style={{ position:'absolute', inset:0, display:'grid', placeItems:'center' }}>
        <div style={{ textAlign:'center' }}>
          <div style={{ fontSize:64, color:'var(--gold)', fontFamily:'Cinzel', animation:'glow-pulse 1.4s ease-in-out infinite' }}>♛</div>
          <div className="t-carved" style={{ color:'var(--gold-bright)', fontSize:10, letterSpacing:'0.4em', marginTop:16 }}>
            CARGANDO EL REINO…
          </div>
        </div>
      </div>
  ...
  return (
    <ModeContext.Provider value={modeCtx}>
      ...
      {screen === 'main' && hero && (
        <>
          {tab === 'board'     && <BoardScreen hero={hero} onOpen={onOpenQuest} onAvatar={() => setTab('profile')} layout={tweaks.boardLayout} density={tweaks.density} completedQuestIds={completedQuestIds} />}
          {tab === 'profile'   && <ProfileScreen hero={hero} user={user} achievements={achievements} onLogout={onLogout} onAvatar={() => setTab('profile')} />}
          {tab === 'inventory' && <InventoryScreen hero={hero} inventory={inventory} />}
          {tab === 'history'   && <HistoryScreen history={history} hero={hero} />}
          {tab === 'rank'      && <GuildScreen user={user} hero={hero} guild={guild} guildId={guildId} onGuildUpdate={onGuildUpdate} />}
          <TabBar tab={tab} onTab={setTab} />
        </>
      )}

      {screen === 'detail' && openQuest && (
        <DetailScreen q={openQuest} hero={hero} onClose={onCloseDetail} onComplete={onCompleteQuest} alreadyCompleted={completedQuestIds.includes(openQuest?.id)} />
      )}
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakSection label="Tablón">
          <TweakRadio label="Disposición" value={tweaks.boardLayout}
            options={[{ value:'rows', label:'Clavadas' }, { value:'scattered', label:'Dispersas' }]}
            onChange={v => setTweak('boardLayout', v)} />
          <TweakRadio label="Densidad" value={tweaks.density}
            options={[{ value:'sparse', label:'Pocas' }, { value:'normal', label:'Normal' }, { value:'dense', label:'Muchas' }]}
...
function RewardOverlay({ reward }) {
  return (
    <div style={{
      ...
      zIndex:200, animation:'float-in 280ms ease-out',
    }}>
        ...
        {reward.leveledUp ? (
          ...
          <>
            <div style={{
              ...
              boxShadow:'0 0 40px rgba(178,40,40,0.6), inset -6px -6px 12px rgba(0,0,0,0.5)',
              display:'grid', placeItems:'center',
              fontFamily:'Cinzel', fontWeight:700, fontSize:32, color:'rgba(0,0,0,0.5)',
              animation:'stamp 700ms cubic-bezier(0.4, 1.5, 0.4, 1)',
            }}>♛</div>
            <div className="t-carved" style={{ fontSize:11, letterSpacing:'0.32em', color:'var(--gold-bright)', marginTop:18 }}>Hazaña sellada</div>
